...
/**
 * @brief 连续输入的帧作为一个sequence，如果数据中断，就创建新的sequence，若发现回环关系，就将新sequence矫正到旧sequence一致的坐标系中
 * 
 */
void new_sequence()
{
...
void relo_relative_pose_callback(const nav_msgs::Odometry::ConstPtr &pose_msg)
{
    ...
    int index = pose_msg->twist.twist.linear.y;//当前帧的id
...
void process()
{
    ...
    while (true)//大循环
    {
        ...
        if(!image_buf.empty() && !point_buf.empty() && !pose_buf.empty())
        {
            ...
            //下面根据pose时间找时间戳同步的原图和地图点，前边确保了第一帧图片时间戳<=point和pose
            else if (image_buf.back()->header.stamp.toSec() >= pose_buf.front()->header.stamp.toSec() 
                && point_buf.back()->header.stamp.toSec() >= pose_buf.front()->header.stamp.toSec())
            {
                pose_msg = pose_buf.front();//取出了pose
                pose_buf.pop();
        ...
        if (pose_msg != NULL)//判断一下是否有效
        {
            ...
            //printf(" image time %f \n", image_msg->header.stamp.toSec());
            // skip fisrt few
            if (skip_first_cnt < SKIP_FIRST_CNT)//跳过开始的10帧
            {
                skip_first_cnt++;
                continue;
            }
            ...
            if((T - last_t).norm() > SKIP_DIS)//要求KF相隔必要的平移距离
            {
                ...
                vector<cv::Point2f> point_2d_uv; //归一化相机坐标系坐标
...
int main(int argc, char **argv)
{
    ...
    // read param
    n.getParam("visualization_shift_x", VISUALIZATION_SHIFT_X);//这两个shift都是0
    n.getParam("visualization_shift_y", VISUALIZATION_SHIFT_Y);
    n.getParam("skip_cnt", SKIP_CNT);//跳过前SKIO_CNT帧
    n.getParam("skip_dis", SKIP_DIS);//两帧距离阈值
    ...
    if (LOOP_CLOSURE)
    {
        ...
        fsSettings["image_topic"] >> IMAGE_TOPIC;     //原图topic   
        ...
        VISUALIZE_IMU_FORWARD = fsSettings["visualize_imu_forward"];//可视化是否进行imu前推
    ...
    std::thread measurement_process;//主处理线程
    std::thread keyboard_command_process;//键盘回调线程

    measurement_process = std::thread(process);//回环检测处理主函数
